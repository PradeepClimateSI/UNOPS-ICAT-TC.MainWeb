...
export class AuditControllerServiceProxy {
    ...
    createCountry(body: AuditDto): Observable<void> {
    ...
    getAuditDetails(page: number, limit: number, userType: string, actionStatus: string, logDate: string, filterText: string, institutionId: number): Observable<void> {
    ...
    protected processGetAuditDetails(response: HttpResponseBase): Observable<void> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
    ...
    getAuditDetailsCountry(page: number, limit: number, userType: string, actionStatus: string, logDate: string, filterText: string, institutionId: number, countryId: number, loginusertype: string, userName: string): Observable<Pagination> {
        let url_ = this.baseUrl + "/audit/audit/auditCountryinfo/{page}/{limit}/{userType}/{actionStatus}/{logDate}/{filterText}/{institutionId}/{countryId}/{loginusertype}/{userName}?";
        if (page === undefined || page === null)
            throw new Error("The parameter 'page' must be defined and cannot be null.");
        else
            url_ += "page=" + encodeURIComponent("" + page) + "&";
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetAuditDetailsCountry(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetAuditDetailsCountry(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<Pagination>;
                }
            } else
                return _observableThrow(response_) as any as Observable<Pagination>;
        }));
    }

    protected processGetAuditDetailsCountry(response: HttpResponseBase): Observable<Pagination> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        let _headers: any = {}; if (response.headers) { for (let key of response.headers.keys()) { _headers[key] = response.headers.get(key); }}
        {
            ...
            resultdefault = Pagination.fromJS(resultDatadefault);
